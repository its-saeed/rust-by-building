...
impl Vec2 {
    ...
    // TODO: add fn normalize(self) -> Vec2
    //   Divide each component by self.length()
}
...
// TODO: define the Collision struct:
// struct Collision {
...
fn overlapping(a: &Body, b: &Body) -> bool {
    ...
    delta.length() < a.radius + b.radius
}

// TODO: replace overlapping() with detect_collision() -> Option<Collision>
//   fn detect_collision(a: &Body, b: &Body) -> Option<Collision> {
//       let delta = b.position + (-a.position);
//       let distance = delta.length();
//       if distance >= a.radius + b.radius { return None; }
//       Some(Collision {
    ...
    loop {
        ...
        for i in 0..world.bodies.len() {
            for j in (i+1)..world.bodies.len() {
                // TODO: replace overlapping() with if let Some(col) = detect_collision(...)
