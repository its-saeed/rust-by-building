...
impl Vec2 {
    ...
    // TODO: add fn dot(self, other: Vec2) -> f32
...
impl World {
    ...
    fn resolve_collisions(&mut self) {
        ...
        for (i, j, col) in &collisions {
            ...
            // TODO: add velocity response here:
            //   1. let vrel = self.bodies[*i].velocity + (-self.bodies[*j].velocity);
            //   2. let vn = vrel.dot(col.normal);
            //   3. if vn <= 0.0 { continue; }   (already separating — skip)
            //   4. let j_val = -(1.0 + 1.0) * vn / 2.0;  (equal mass, e=1)
            //   5. self.bodies[*i].velocity = self.bodies[*i].velocity + col.normal * j_val;
            //   6. self.bodies[*j].velocity = self.bodies[*j].velocity + (-col.normal) * j_val;
        }
    }
