...
Open `lessons/7-pong/lesson-06/project/src/main.rs`.
...
6. In the `ScoreEvent::Point` and `ScoreEvent::GameOver` arms, play `score_sound`.

```sh
cargo run --bin pong-06
```

Put two WAV files in `lessons/7-pong/lesson-06/project/assets/` before running. Play a rally — the ball should get noticeably faster after a few hits, and each event should produce a sound.
